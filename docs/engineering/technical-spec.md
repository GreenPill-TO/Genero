...
- The Foundry helper layer now includes a read-only Mento discovery script that queries `Broker.getExchangeProviders()` plus each provider’s `getExchanges()` output to resolve live `exchangeId` values from chain state. The checked-in Celo mainnet config records the verified `USDC -> USDm` and `USDm -> CADm` exchange IDs, while the current single-hop adapter deployment path still seeds `USDm -> CADm` as the active route.
