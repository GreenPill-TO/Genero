...
- **Storage**: Supabase (Postgres + Auth)
  ...
  - Local Supabase startup now has an explicit repo helper, `pnpm supabase:start:local`, which starts the trimmed Colima-backed stack and reapplies `GOTRUE_MAILER_EXTERNAL_HOSTS=localhost,127.0.0.1,kong` to the GoTrue container because the current Supabase CLI `config.toml` surface does not expose that mailer-host setting.
...
- **API Routes**: Custom `/api/auth/sms` for Twilio verification, wallet auth, and onboarding.
  ...
  - The city-manager stores contract now carries a richer applicant summary directly in the list response, including username, phone, country, address, profile image URL, and account creation timestamp, so the operator workspace can open an applicant inspector without issuing a second browser fetch.
  ...
  - `wallet-operations` now also owns empty-contacts invite onboarding: `/contacts/imports` persists app-scoped contact-import approval plus deduplicated imported email contacts, and `/contacts/invite-batches` stores queued invite subjects/messages plus recipient rows for downstream email processing.
  - The invite-onboarding persistence layer is split deliberately by concern: `public.user_contact_import_preferences` stores the signed-in user’s import choice per app instance, `public.user_imported_contacts` stores reusable imported email contacts, and `public.contact_invite_batches` plus `public.contact_invite_batch_recipients` capture the edited outbound invite batch state without pretending the wallet sent mail synchronously.
...
- `user_identifier` is now created by the canonical user-settings write paths rather than left implicit: authenticated-user resolution lazily backfills missing identifiers, profile updates upgrade fallback `user-<id>` identifiers to username-based ones when possible, and wallet custody registration performs the same check as a final safety net after step 5 wallet setup.
...
- Wallet `/welcome` now composes its start, resume, signed-out, loading, and fallback states from the shared authenticated wallet tokens (`walletPageClass`, `walletPanelClass`, `walletPanelMutedClass`, `walletBadgeClass`) instead of the public card chrome, keeping onboarding visually aligned with the dashboard shell.
- The shared wallet `ContentLayout` now centres its initial auth-loading placeholder with flex alignment and a dedicated `wallet-layout-loading` test hook, so `http://localhost:3000/` no longer paints `...loading` in the top-left corner before auth settles.
- The wallet landing page copy in `app/tcoin/wallet/page.tsx` now uses the updated public-language contract for physical notes, QR recipients, merchant fee framing, and TCOIN governance/backing, keeping those public promises aligned with the current product narrative.
- Wallet `/welcome` now separates product orientation from signup instructions: the pre-start card carries the Toronto/TTC/charity framing copy, while wizard step 1 is the dedicated resumable-signup explainer.
- Wallet onboarding step 1 now leads with direct action copy (`Let's ...`) rather than the earlier meta explanation string, keeping the first wizard panel shorter and less repetitive after the product-introduction card.
- Wallet onboarding step 2 now renders two muted panels with explicit section titles for required vs optional data, uses empty first/last-name fields with `Mats` / `Sundin` placeholders for new signups instead of preloading bootstrap name values, and only blocks continuation on first name, last name, and phone verification.
- Wallet onboarding step 3 now prepares selected images locally and opens a dedicated `WelcomeProfilePictureEditorModal` that reuses the circular crop model (zoom plus horizontal/vertical offsets) before saving the cropped avatar through the existing profile-picture upload path.
- The shared modal shell now omits empty title/description blocks, which allows onboarding’s picture-editor modal to render as content-only chrome without blank modal headers.
- Wallet onboarding profile-picture saves now map low-level storage/network failures such as `name resolution failed` into a user-facing upload message that explains the photo could not be uploaded yet and can be added later from Edit Profile.
- The welcome-page reset handler now explicitly closes the wizard client state after `resetSignup()`, so a reset draft remains on the step-0 welcome card instead of briefly showing it and then auto-forwarding back into step 1.
...
- Edit Profile now accepts any image aspect ratio, prepares a local crop model in `shared/lib/profilePictureCrop.ts`, lets the user position and zoom the image inside a circular preview, and uploads the resulting cropped square PNG instead of the undeformed source file.
- The Edit Profile avatar UI now computes separate crop frames for the small profile-summary preview and the larger editor preview, so both circles render the same crop state at different sizes instead of reusing one pixel frame across two containers.
- Shared avatar rendering now uses `object-cover`, so existing non-square image URLs fill circular avatar shells without stretching elsewhere in the wallet chrome.
- `shared/lib/profilePictureCrop.ts` now exposes `prepareProfilePictureFromUrl`, which the wallet Edit Profile modal uses to pull an existing public avatar back into the same local crop pipeline used for new uploads.
- The Edit Profile country picker now uses an explicit search-driven `react-select` flow: the field label is “Country or Country number”, the menu stays closed until the user types, and filtering matches both display labels and country codes instead of showing guessed options on focus.
- Shared app-facing form controls now derive from one surface token in `shared/components/ui/formFieldStyles.ts`, giving text inputs, textareas, select triggers, command-search inputs, file pickers, and OTP digit fields the same non-transparent background treatment; wallet/sparechange onboarding and settings screens that still use raw HTML fields now opt into that same surface explicitly.
- The wallet Edit Profile modal now groups its personal fields into a `Banking info` card for legal/account-holder details (`Given name(s)`, surname, country, phone) and an `Info used in this app` card for wallet-local identity settings (`username`, preferred name).
- Wallet Edit Profile now breaks into four panels (`Picture`, `Email`, `Banking info`, `Info used in this app`) inside a `lg` two-column grid, and the authenticated modal system now supports a dedicated `5xl` size so that modal can use a wider desktop shell without inflating smaller dialogs.
- Wallet account emails now live in `public.user_email_addresses`, with soft deletes (`deleted_at`) for historical ownership periods, a partial unique index that only constrains active emails, and a partial unique index that enforces exactly one active primary email per user.
- The wallet user-settings bootstrap now returns both `user.email` as the compatibility primary-email mirror and `user.emails` as the active managed-email list, while profile updates accept an `emails[]` payload and reconcile inserts, primary switches, and soft deletes into `public.user_email_addresses`.
- Auth-side wallet user resolution now checks active rows in `public.user_email_addresses` before falling back to `users.email`, and the ensure-user flow preserves the authenticated email in the managed-email table without overwriting the rest of the user’s active email list.
- The More-tab Account centre heading now prefers the saved preferred name (`nickname`) when present, falls back to the given-name field next, and only uses the fuller legacy name fields after those account-level values are absent.
- The authenticated navbar account widget now prefers the normalized user-settings bootstrap for identity chrome: it uses the saved profile image when available and summarizes the account with preferred name plus email in both the closed trigger and the opened dropdown.
