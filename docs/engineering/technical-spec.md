...
- `MoreTab` now derives a post-migration `public.users` column list from the current schema contract and renders a bottom account-record section that maps authenticated bootstrap/Cubid/auth-session fields into that column order, formatting booleans and JSON values while still rendering empty rows.
