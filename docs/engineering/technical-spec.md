...
- Operational constraint: the current `GeneroTokenV3` visible/base conversion path still cannot safely support very large single-account balances. In live TorontoCoin routing, seeding a pool account far above roughly `9.22` visible `cplTCOIN` causes `balanceOf(poolAccount)` to revert, which in turn breaks `ManagedPoolAdapter` inventory reads and router previews. Large-liquidity pool support therefore still requires a token-math fix or a different pool-inventory model.
...
- `ManagedPoolAdapter` is now the deployable production `poolAdapter` implementation for TorontoCoin retail routing. It owns canonical pool settlement accounts, quote-bps execution settings, pool inventory custody, and merchant-to-pool matching, while `PoolRegistry` remains the canonical identity and activity registry.
...
- Live operational note: after the large-balance `cplTCOIN` limit surfaced on Celo mainnet, the retail router path was recovered by switching `LiquidityRouter` to a fresh `ManagedPoolAdapter` and reseeding the bootstrap pool with a small safe inventory. Until the token math is patched, adapter-managed pool accounts must stay below that single-account balance ceiling.
...
- A live Celo mainnet migration attempt proved that token replacement alone is insufficient: `TreasuryController`, `LiquidityRouter`, and `ManagedPoolAdapter` still scale internal token amounts as if `mrTCOIN` and `cplTCOIN` were `18` decimals. Swapping only the token addresses to fresh `6`-decimal deployments causes pool-liquidity checks to revert before cutover, so a real live migration now requires code-level scaling fixes in the controller/router/pool-accounting path.
- The workspace now includes explicit mainnet migration tooling for that future cutover under `script/deploy/`: `StageTorontoCoinSixDecimalMigration.s.sol`, `ProposeTorontoCoinSixDecimalMigration.s.sol`, `FinalizeTorontoCoinSixDecimalMigration.s.sol`, and `AbortTorontoCoinSixDecimalMigration.s.sol`. The first live run staged fresh `6`-decimal tokens and a fresh pool adapter, then cancelled proposals `17`, `18`, and `19` after the scaling mismatch was confirmed, leaving the live TorontoCoin mainnet pointers unchanged.
