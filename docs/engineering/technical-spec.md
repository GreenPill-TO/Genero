...
- `ReserveInputRouter` is now the dedicated reserve-input normalization helper for retail `cplTCOIN` acquisition: it checks whether `tokenIn` is already treasury-accepted, and only engages the Mento-style swap path into `mCAD` when direct reserve acceptance is unavailable.
- `MentoBrokerSwapAdapter` is now the concrete `ISwapAdapter` implementation for Mento execution. It stores default broker routes on-chain per input token, supports optional per-call route overrides through `swapData`, and can be shared by both `ReserveInputRouter` and the older `TcoinMintRouter`.
- `LiquidityRouter` remains the execution layer for `cplTCOIN` purchases: it pulls the user’s input token, normalizes it only when needed through `ReserveInputRouter`, asks `TreasuryController` for mrTCOIN-equivalent route capacity, reads the buyer’s canonical acceptance preferences on-chain, hard-excludes denied pools and denied merchant ecosystems, respects strict token/pool/merchant acceptance, scores active pools with governance-set weights plus ranked merchant bonuses, falls back automatically to the best eligible pool, and mints a configurable `cplTCOIN` charity top-up directly to the buyer’s resolved charity wallet.
- Foundry deployment wiring now has a dedicated liquidity-stack script that deploys `MentoBrokerSwapAdapter`, `ReserveInputRouter`, and `LiquidityRouter`, resolves their circular helper/router dependency, wires `TreasuryController.setLiquidityRouter(...)`, optionally seeds one initial Mento route, and then transfers ownership to governance.
- The Foundry workspace now standardizes deploy-target selection around `DEPLOY_TARGET_CHAIN` (`celo` or `sepolia`), `MAINNET_RPC_URL` / `SEPOLIA_RPC_URL`, and separate explorer-key envs (`CELOSCAN_API_KEY` for Celo mainnet and `ETHERSCAN_API_KEY` for Sepolia). Deploy/admin scripts validate that the selected target matches the broadcast chain ID before continuing.
- The Foundry helper layer now includes a read-only Mento discovery script that queries `Broker.getExchangeProviders()` plus each provider’s `getExchanges()` output to resolve live `exchangeId` values for pairs such as `USDC -> mCAD`, instead of relying on hardcoded guesses.
