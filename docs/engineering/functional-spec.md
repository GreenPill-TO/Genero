...
- **Operator status surface**: wallet operators now have one read-only TorontoCoin ops status surface covering current live suite addresses, governance ownership checks, bootstrap pool liquidity, a canonical preview quote, reserve-route health, and validator timestamps. The same contract/runtime data is available through both the wallet admin panel and the checked-in ops script/runbook.
