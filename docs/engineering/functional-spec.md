...
- The Receive tab now generates its QR payload as soon as the wallet identifier is present, even if the numeric profile id has not finished loading, and shows an explicit unavailable message instead of a permanent “Loading QR Code...” state when that identifier is missing.
