...
- **Manual external-pool onboarding**: external Sarafu pools can now be added to TorontoCoin mainnet, but the current governance proposal catalogue still covers `addPool` without covering `setPoolAddress`. Operators should therefore treat onboarding as a two-part on-chain procedure: propose and execute `addPool`, then bind the live `SwapPool` address through the narrow temporary governance helper and restore the standard helper immediately after.
