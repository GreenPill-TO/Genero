...
| Worker and privileged function runtime | `SUPABASE_SERVICE_ROLE_KEY`, `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` | The deployed Next.js wallet runtime does not need the service-role key. Edge Functions need the publishable key for request-scoped self-service RPCs and scoped identity/app-context resolution. The service-role key remains required only for route-specific privileged Edge mutations and the external indexer touch worker (`pnpm ops:indexer:drain-touch-queue`). |
...
5. Confirm the target Supabase project has both release read migrations applied and PostgREST reloaded:
...
When using one of the ignored local Supabase target profiles, wrap the TorontoCoin ops commands with the profile launcher, for example:

```bash
pnpm exec tsx scripts/run-with-env-profile.ts .env.local-supabase-remote -- pnpm ops:torontocoin
pnpm exec tsx scripts/run-with-env-profile.ts .env.local-supabase-remote -- pnpm ops:torontocoin:pools
```
