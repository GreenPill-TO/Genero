...
contract MockTreasuryControllerForRouter is ITreasuryControllerForLiquidityRouter {
        ...
        returns (uint256 mrTcoinOut)
    {
        ...
        MockERC20(token).transferFrom(msg.sender, address(this), assetAmount);
...
contract LiquidityRouterTest is Test {
    ...
    function setUp() public {
        ...
        treasury = new MockTreasuryControllerForRouter();
        charityPreferences = new MockCharityPreferencesForRouter();
        poolRegistry = new MockPoolRegistryForRouter();
        poolAdapter = new MockPoolAdapterForRouter(address(cplToken));
    ...
    function test_BuyCplTcoinHonoursEligiblePreferredPoolAndMintsCharityTopup() public {
        ...
        assertEq(reserveToken.balanceOf(address(treasury)), 100e18);
    }
    ...
    function _buy(
        ...
        ) = router.buyCplTcoin(RESERVE_ASSET_ID, reserveAssetAmount, minCplTcoinOut, preferredPools, preferredMerchants);
    }

    function _preview(uint256 reserveAssetAmount, bytes32[] memory preferredPools, bytes32[] memory preferredMerchants)
        internal
        view
