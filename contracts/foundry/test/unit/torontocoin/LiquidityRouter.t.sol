...
contract LiquidityRouterTest is Test {
    ...
    function test_BuyCplTcoinUsesStoredAcceptedPoolPreferenceAndMintsCharityTopup() public {
        ...
        PreviewResult memory preview = _preview(USER, 100e18);
        ...
        vm.prank(USER);
        BuyResult memory purchase = _buy(100e18, 100e18);
    ...
    function test_DeniedMerchantHardExcludesMatchingPool() public {
        vm.prank(USER);
        acceptanceRegistry.setMerchantAcceptance(MERCHANT_B, UserAcceptancePreferencesRegistry.AcceptanceStatus.Denied);
    ...
    function test_DenyingCplTcoinMakesAllRoutesIneligible() public {
        vm.prank(USER);
        acceptanceRegistry.setTokenAcceptance(address(cplToken), UserAcceptancePreferencesRegistry.AcceptanceStatus.Denied);
    ...
    function _buy(uint256 reserveAssetAmount, uint256 minCplTcoinOut) internal returns (BuyResult memory result) {
        (
            result.selectedPoolId, result.mrUsed, result.cplOut, result.charityTopupOut, result.resolvedCharityId
        ) = router.buyCplTcoin(RESERVE_ASSET_ID, reserveAssetAmount, minCplTcoinOut);
    }

    function _preview(address buyer, uint256 reserveAssetAmount) internal view returns (PreviewResult memory result) {
        (
        ...
        ) = router.previewBuyCplTcoin(buyer, RESERVE_ASSET_ID, reserveAssetAmount);
    }
}
